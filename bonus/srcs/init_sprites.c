...
/*   Updated: 2025/07/30 17:45:11 by a-soeiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sprite->image = mlx_xpm_file_to_image(mlx_ptr, filepath, &sprite->x, &sprite->y);
...
void	init_samus_sprites(t_game *game)
{
	...
	while (i < 6)
	{
		load_sprite(game->mlx_ptr, &game->samus.sprite[i], samus_sprite_paths[i]);
		i++;
	}
}
